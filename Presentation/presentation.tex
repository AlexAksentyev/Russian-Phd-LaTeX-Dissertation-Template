...
\begin{frame}{Рассматриваемая структура}
	\begin{tikzpicture}
		...
		\pause
		\node (tilt) at (lat.center)[xshift=2.5cm,yshift=-1.6cm] {\includegraphics[width=.77\linewidth,trim = 30 0 0 0, clip]{images/magnet_tilting}};
		\pause
...
\begin{frame}{Смена полярности ведущего поля}
	...
	\begin{tikzpicture}
		...
		\pause
		\node (first) at (null.center) {\includegraphics[height=.7\paperheight, trim=5 145 0 135, clip]{images/magnet_tilted-1.pdf}};
		\pause
		\node (final) at (first.center) {\includegraphics[height=.7\paperheight, trim=5 145 0 135, clip]{images/magnet_tilted-2.pdf}};
		\pause
		\node (rev) at (final.center) {\includegraphics[height=.7\paperheight, trim=5 145 0 135, clip]{images/magnet_tilted-3.pdf}};
		\node (eq) at (rev.south east)[yshift=.4cm] {$\hat\W_{edm} = \frac12\bkt{\W_x^{CW} + \W_x^{CCW}}$};
	\end{tikzpicture}
\end{frame}
